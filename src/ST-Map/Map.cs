...
public class Map
{
    ...
    // Map bonus zones
    public Vector[] BonusStartZone { get; } =
        Enumerable.Repeat(0, 99).Select(x => new Vector(0, 0, 0)).ToArray(); // To-do: Implement bonuses

    public QAngle[] BonusStartZoneAngles { get; } =
        Enumerable.Repeat(0, 99).Select(x => new QAngle(0, 0, 0)).ToArray(); // To-do: Implement bonuses

    public Vector[] BonusEndZone { get; } =
        Enumerable.Repeat(0, 99).Select(x => new Vector(0, 0, 0)).ToArray(); // To-do: Implement bonuses
    ...
    public Map(string Name, TimerDatabase database)
    {
        ...
        IEnumerable<CTriggerTeleport> teleports =
            Utilities.FindAllEntitiesByDesignerName<CTriggerTeleport>("info_teleport_destination");
            ...
            if (trigger.Entity!.Name != null)
            {
                    ...
                    trigger.Entity!.Name.Contains("s1_start"))
                {
                    ...
                    foreach (CBaseEntity teleport in teleports)
                    {
                        ...
                        {
                            this.StartZone = new Vector(teleport.AbsOrigin!.X, teleport.AbsOrigin!.Y,
                                teleport.AbsOrigin!.Z);
                            this.StartZoneAngles = new QAngle(teleport.AbsRotation!.X, teleport.AbsRotation!.Y,
                                teleport.AbsRotation!.Z);
                ...
                else if (Regex.Match(trigger.Entity.Name, "^s([1-9][0-9]?|tage[1-9][0-9]?)_start$").Success)
                {
                    ...
                    foreach (CBaseEntity teleport in teleports)
                    {
                        if (teleport.Entity!.Name != null &&
                            (IsInZone(trigger.AbsOrigin!, trigger.Collision.BoundingRadius, teleport.AbsOrigin!) ||
                             (Regex.Match(teleport.Entity.Name, "^spawn_s([1-9][0-9]?|tage[1-9][0-9]?)_start$")
                                     .Success && Int32.Parse(Regex.Match(teleport.Entity.Name, "[0-9][0-9]?").Value) ==
                                 stage)))
                        {
                            this.StageStartZone[stage - 1] = new Vector(teleport.AbsOrigin!.X, teleport.AbsOrigin!.Y,
                                teleport.AbsOrigin!.Z);
                            this.StageStartZoneAngles[stage - 1] = new QAngle(teleport.AbsRotation!.X,
                                teleport.AbsRotation!.Y, teleport.AbsRotation!.Z);
                    ...
                    if (!foundPlayerSpawn)
                    {
                        this.StageStartZone[stage - 1] = new Vector(trigger.AbsOrigin!.X, trigger.AbsOrigin!.Y,
                            trigger.AbsOrigin!.Z);
                    }
                }
                ...
                {
                    this.CheckpointStartZone[Int32.Parse(Regex.Match(trigger.Entity.Name, "[0-9][0-9]?").Value) - 1] =
                        new Vector(trigger.AbsOrigin!.X, trigger.AbsOrigin!.Y, trigger.AbsOrigin!.Z);
                ...
                else if (Regex.Match(trigger.Entity.Name, "^b([1-9][0-9]?|onus[1-9][0-9]?)_start$").Success)
                {
                    ...
                    foreach (CBaseEntity teleport in teleports)
                    {
                        if (teleport.Entity!.Name != null &&
                            (IsInZone(trigger.AbsOrigin!, trigger.Collision.BoundingRadius, teleport.AbsOrigin!) ||
                             (Regex.Match(teleport.Entity.Name, "^spawn_b([1-9][0-9]?|onus[1-9][0-9]?)_start$")
                                     .Success && Int32.Parse(Regex.Match(teleport.Entity.Name, "[0-9][0-9]?").Value) ==
                                 bonus)))
                        {
                            this.BonusStartZone[bonus - 1] = new Vector(teleport.AbsOrigin!.X, teleport.AbsOrigin!.Y,
                                teleport.AbsOrigin!.Z);
                            this.BonusStartZoneAngles[bonus - 1] = new QAngle(teleport.AbsRotation!.X,
                                teleport.AbsRotation!.Y, teleport.AbsRotation!.Z);
                    ...
                    if (!foundPlayerSpawn)
                    {
                        this.BonusStartZone[bonus - 1] = new Vector(trigger.AbsOrigin!.X, trigger.AbsOrigin!.Y,
                            trigger.AbsOrigin!.Z);
                    }
                }
                ...
                {
                    this.BonusEndZone[Int32.Parse(Regex.Match(trigger.Entity.Name, "[0-9][0-9]?").Value) - 1] =
                        new Vector(trigger.AbsOrigin!.X, trigger.AbsOrigin!.Y, trigger.AbsOrigin!.Z);
                }
            }
        }
        ...
        if (mapData.HasRows &&
            mapData.Read()) // In here we can check whether MapData in DB is the same as the newly extracted data, if not, update it (as hookzones may have changed on map updates)
        {
        ...
        else
        {
            ...
            if (writerRows != 1)
                throw new Exception(
                    $"CS2 Surf ERROR >> OnRoundStart -> new Map() -> Failed to write new map to database, this shouldn't happen. Map: {Name}");
            writer.Dispose();
        ...
        string query =
            $"UPDATE Maps SET last_played={(int)DateTimeOffset.UtcNow.ToUnixTimeSeconds()} WHERE id={this.ID}";
        if (updateData)
            query =
        ...
        if (lastPlayedUpdateRows != 1)
            throw new Exception(
                $"CS2 Surf ERROR >> OnRoundStart -> update Map() -> Failed to update map in database, this shouldnt happen. Map: {Name} | was it 'big' update? {updateData}");
        updater.Dispose();
    ...
    {
        if (spawnOrigin.X >= zoneOrigin.X - zoneCollisionRadius &&
            spawnOrigin.X <= zoneOrigin.X + zoneCollisionRadius &&
            spawnOrigin.Y >= zoneOrigin.Y - zoneCollisionRadius &&
            spawnOrigin.Y <= zoneOrigin.Y + zoneCollisionRadius &&
    ...
    internal void GetMapRecordAndTotals(TimerDatabase DB, int style = 0) // To-do: Implement styles
    {
        ...
        if (mapWrData.HasRows)
        {
            ...
            {
                if (totalRows ==
                    0) // We are sorting by `run_time ASC` so the first row is always the fastest run for the map and style combo :)
                {
                    this.WR[style].ID = mapWrData.GetInt32("id"); // WR ID for the Map and Style combo
                    this.WR[style].Ticks =
                        mapWrData.GetInt32("run_time"); // Fastest run time (WR) for the Map and Style combo
                    this.WR[style].StartVelX =
                        mapWrData.GetFloat("start_vel_x"); // Fastest run start velocity X for the Map and Style combo
                    this.WR[style].StartVelY =
                        mapWrData.GetFloat("start_vel_y"); // Fastest run start velocity Y for the Map and Style combo
                    this.WR[style].StartVelZ =
                        mapWrData.GetFloat("start_vel_z"); // Fastest run start velocity Z for the Map and Style combo
                    this.WR[style].EndVelX =
                        mapWrData.GetFloat("end_vel_x"); // Fastest run end velocity X for the Map and Style combo
                    this.WR[style].EndVelY =
                        mapWrData.GetFloat("end_vel_y"); // Fastest run end velocity Y for the Map and Style combo
                    this.WR[style].EndVelZ =
                        mapWrData.GetFloat("end_vel_z"); // Fastest run end velocity Z for the Map and Style combo
                    this.WR[style].RunDate =
                        mapWrData.GetInt32("run_date"); // Fastest run date for the Map and Style combo
                    this.WR[style].Name =
                        mapWrData.GetString("name"); // Fastest run player name for the Map and Style combo
                }
        ...
        this.TotalCompletions =
            totalRows; // Total completions for the map and style - this should maybe be added to PersonalBest class

        // Get map world record checkpoints
        if (totalRows != 0)
        {
            Task<MySqlDataReader> cpReader =
                DB.Query($"SELECT * FROM `Checkpoints` WHERE `maptime_id` = {this.WR[style].ID};");
            ...
            while (cpWrData.Read())
            {
                ...
                Checkpoint cp = new(cpWrData.GetInt32("cp"),
                    cpWrData.GetInt32(
                        "run_time"), // To-do: what type of value we use here? DB uses DECIMAL but `.Tick` is int???
...
#if DEBUG
                Console.WriteLine(
                    $"======= CS2 Surf DEBUG >> internal void GetMapRecordAndTotals : Map -> Loaded WR CP {cp.CP} with RunTime {cp.Ticks} for MapTimeID {WR[0].ID} (MapId = {this.ID}).");
#endif
            }
