...
public class PlayerHUD
{
    ...
    public void Display()
    {
        ...
        if (_player.Controller.PawnIsAlive)
        {
            ...
            float velocity = (float)Math.Sqrt(
                _player.Controller.PlayerPawn.Value!.AbsVelocity.X * _player.Controller.PlayerPawn.Value!.AbsVelocity.X
                + _player.Controller.PlayerPawn.Value!.AbsVelocity.Y *
                _player.Controller.PlayerPawn.Value!.AbsVelocity.Y
                + _player.Controller.PlayerPawn.Value!.AbsVelocity.Z *
                _player.Controller.PlayerPawn.Value!.AbsVelocity.Z);
            ...
            {
                rankModule = FormatHUDElementHTML("Rank",
                    $"{_player.Stats.PB[style].Rank}/{_player.CurrMap.TotalCompletions}", "#7882dd");
            }
            ...
            // PB & WR Modules
            string pbModule = FormatHUDElementHTML("PB",
                _player.Stats.PB[style].Ticks > 0 ? FormatTime(_player.Stats.PB[style].Ticks) : "N/A",
                "#7882dd"); // IMPLEMENT IN PlayerStats // To-do: make Style (currently 0) be dynamic
            string wrModule = FormatHUDElementHTML("WR",
                _player.CurrMap.WR[style].Ticks > 0 ? FormatTime(_player.CurrMap.WR[style].Ticks) : "N/A",
                "#ffc61a"); // IMPLEMENT IN PlayerStats - This should be part of CurrentMap, not PlayerStats?

            var buttons = _player.Controller.Buttons;
            List<string> buttonDisplay = new List<string>
            {
                $"{((buttons & PlayerButtons.Jump) != 0 ? "JUMP" : "_")}",
                $"{((buttons & PlayerButtons.Duck) != 0 ? "DUCK" : "_")}",
            ...
            string buttonModule = FormatHUDElementHTML("", string.Join(" ", buttonDisplay), "#ff4d00");
            
            // Build HUD
            string hud = $"{timerModule}<br>{velocityModule}<br>{pbModule} | {rankModule}<br>{wrModule}<br>{buttonModule}";
        ...
        else if (_player.Controller.Team == CsTeam.Spectator)
        {
            ...
            {
                if (!_player.CurrMap.ReplayBots[i].IsPlayable ||
                    !_player.IsSpectating(_player.CurrMap.ReplayBots[i].Controller!))
                    continue;

                string replayModule = $"{FormatHUDElementHTML("", "REPLAY", "red", "large")}";

                string nameModule = FormatHUDElementHTML($"{_player.CurrMap.ReplayBots[i].Stat_PlayerName}",
                    $"{FormatTime(_player.CurrMap.ReplayBots[i].Stat_RunTime)}", "#ffd500");

                string elapsed_time = FormatHUDElementHTML("Time",
                    $"{PlayerHUD.FormatTime(_player.CurrMap.ReplayBots[i].Stat_RunTick)}", "#7882dd");
                
                var buttons = _player.CurrMap.ReplayBots[i].Controller!.Buttons;
                List<string> buttonDisplay = new List<string>
                {
                    $"{((buttons & PlayerButtons.Jump) != 0 ? "JUMP" : "_")}",
                    $"{((buttons & PlayerButtons.Duck) != 0 ? "DUCK" : "_")}",
                ...
                string hud = $"{replayModule}<br>{elapsed_time}<br>{nameModule}<br>{buttonModule}";
    ...
    public void
        DisplayCheckpointMessages(
            string PluginPrefix) // To-do: PluginPrefix should be accessible in here without passing it as a parameter
    {
        int pbTime;
        int wrTime = -1;
        float pbSpeed;
        ...
        try
        {
            ...
            pbSpeed = (float)Math.Sqrt(
                _player.Stats.PB[style].Checkpoint[_player.Timer.Checkpoint].StartVelX *
                _player.Stats.PB[style].Checkpoint[_player.Timer.Checkpoint].StartVelX
                + _player.Stats.PB[style].Checkpoint[_player.Timer.Checkpoint].StartVelY *
                _player.Stats.PB[style].Checkpoint[_player.Timer.Checkpoint].StartVelY
                + _player.Stats.PB[style].Checkpoint[_player.Timer.Checkpoint].StartVelZ *
                _player.Stats.PB[style].Checkpoint[_player.Timer.Checkpoint].StartVelZ);

#if DEBUG
            Console.WriteLine(
                $"CS2 Surf DEBUG >> DisplayCheckpointMessages -> [TIME]  Got pbTime from _player.Stats.PB[{style}].Checkpoint[{_player.Timer.Checkpoint} = {pbTime}]");
            Console.WriteLine(
                $"CS2 Surf DEBUG >> DisplayCheckpointMessages -> [SPEED] Got pbSpeed from _player.Stats.PB[{style}].Checkpoint[{_player.Timer.Checkpoint}] = {pbSpeed}");
#endif
        }
#if DEBUG
...
#if DEBUG
            Console.WriteLine(
                $"CS2 Surf CAUGHT EXCEPTION >> DisplayCheckpointMessages -> An error occurred: {ex.Message}");
            Console.WriteLine(
                $"CS2 Surf CAUGHT EXCEPTION >> DisplayCheckpointMessages -> An error occurred Player has no PB and therefore no Checkpoints | _player.Stats.PB[{style}].Checkpoint.Count = {_player.Stats.PB[style].Checkpoint.Count}");
#endif
        }
        ...
        if (pbTime != -1)
        {
            ...
            if (pbTime - currentTime < 0.0)
            {
                strPbDifference +=
                    ChatColors.Red + "+" +
                    FormatTime((pbTime - currentTime) * -1); // We multiply by -1 to get the positive value
            }
            ...
            if (pbSpeed - currentSpeed <= 0.0)
            {
                strPbDifference +=
                    "(" + ChatColors.Green + "+" +
                    ((pbSpeed - currentSpeed) * -1).ToString("0"); // We multiply by -1 to get the positive value
            }
        ...
        if (_player.CurrMap.WR[style].Ticks > 0)
        {
...
#if DEBUG
            Console.WriteLine(
                $"CS2 Surf DEBUG >> DisplayCheckpointMessages -> Starting WR difference calculation... (_player.CurrMap.WR[{style}].Ticks > 0)");
#endif

            wrTime = _player.CurrMap.WR[style].Checkpoint[_player.Timer.Checkpoint].Ticks;
            wrSpeed = (float)Math.Sqrt(
                _player.CurrMap.WR[style].Checkpoint[_player.Timer.Checkpoint].StartVelX *
                _player.CurrMap.WR[style].Checkpoint[_player.Timer.Checkpoint].StartVelX
                + _player.CurrMap.WR[style].Checkpoint[_player.Timer.Checkpoint].StartVelY *
                _player.CurrMap.WR[style].Checkpoint[_player.Timer.Checkpoint].StartVelY
                + _player.CurrMap.WR[style].Checkpoint[_player.Timer.Checkpoint].StartVelZ *
                _player.CurrMap.WR[style].Checkpoint[_player.Timer.Checkpoint].StartVelZ);
            // Reset the string
            strWrDifference = "";
            ...
            if (wrTime - currentTime < 0.0)
            {
                strWrDifference +=
                    ChatColors.Red + "+" +
                    FormatTime((wrTime - currentTime) * -1); // We multiply by -1 to get the positive value
            }
            ...
            if (wrSpeed - currentSpeed <= 0.0)
            {
                strWrDifference +=
                    "(" + ChatColors.Green + "+" +
                    ((wrSpeed - currentSpeed) * -1).ToString("0"); // We multiply by -1 to get the positive value
            }
